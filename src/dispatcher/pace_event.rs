...
use crate::{cache::{guild_data::GuildData, players::PlayerSplitsData}, eprintln, utils::{format_time::format_time, millis_to_mins_secs::millis_to_hrs_mins}, ws::response::{Advancement, Response}};

use super::{consts::{OFFLINE_EMOJI, SPECIAL_UNDERSCORE, TWITCH_EMOJI}, get_run_info::get_run_info};

pub async fn handle_pace_event(ctx: Arc<Context>, response: &Response, live_link: String, stats_link: String, author: CreateEmbedAuthor, last_advancement: &Advancement, guild_data: &mut GuildData) 
{
        let run_info = 
            match get_run_info(response, last_advancement) {
                Some(info) => info,
                None => {
                    return eprintln!("HandlePaceEvent: Unrecognized event id: {:#?}.", last_advancement.event_id);
                }
            };
