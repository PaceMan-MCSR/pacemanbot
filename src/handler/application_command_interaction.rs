...
use crate::{
    components::application::{
        send_role_selection_message::send_role_selection_message, setup_pb_roles::setup_pb_roles,
...
pub async fn handle_application_command_interaction(
    ...
    match match command.data.name.as_str() {
        "send_message_17" => send_role_selection_message(&ctx, &roles, command).await,
        "setup_pings_17" => setup_pings(&ctx, guild_id, command).await,
        "setup_roles_17" => setup_roles(&ctx, guild_id, command).await,
        "setup_pb_roles_17" => setup_pb_roles(&ctx, guild_id, command).await,
        "whitelist_17" => whitelist(&ctx, guild_id, command).await,
        "validate_config_17" => validate_config(&ctx, guild_id, command).await,
        _ => {
            return Err(format!(
